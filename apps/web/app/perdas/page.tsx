...
import {
  createDemoLossEntries,
  demoWorkflowLossTypes,
...
export default function LossesPage() {
  ...
  async function loadData(nextWeekId = weekId) {
    ...
    try {
      if (DEMO_MODE) {
        ...
        setLossTypeId(
          (current) => current || demoWorkflowLossTypes[0]?.id || "",
        );
        setEntries((current) =>
          current.length ? current : createDemoLossEntries(),
        );
        setAlerts(demoDashboardAlerts);
        setMessage(
          "Perdas demonstrativas carregadas; o workflow funciona localmente.",
        );
        return;
      }
      const [weekRows, typeRows, productRows, equipmentRows, shiftRows] =
        await Promise.all([
      ...
      if (selectedWeek) {
        const [lossRows, nextAlerts] = await Promise.all([
          apiGetClient<LossEntryRow[]>(
            `/losses?weekId=${encodeURIComponent(selectedWeek)}`,
          ),
        ...
        setMessage(
          weekRows.length
            ? "Selecione uma semana para carregar as perdas."
      ...
      setMessage(
        error instanceof Error
          ? error.message
          : "Nao foi possivel carregar perdas da API.",
      );
    } finally {
      setLoading(false);
    }
  ...
  async function saveLoss() {
    if (!session || !weekId || !lossTypeId || !productId) {
      setMessage(
        "Entre no sistema e selecione semana, tipo e produto com preco aprovado.",
      );
      return;
    }
    if (DEMO_MODE) {
      ...
      setMessage(
        "Rascunho de perda criado localmente. Use Enviar para iniciar a aprovação demonstrativa.",
      );
      return;
    }
    setLoading(true);
    try {
      ...
      setMessage(
        error instanceof Error ? error.message : "Falha ao salvar perda.",
      );
    } finally {
      setLoading(false);
    }
  ...
  const approvedEntries = entries.filter(
    (entry) => entry.workflowStatus === "APPROVED",
  );
  const total = approvedEntries.reduce(
  ...
  const optionalKg = (value: string | number | null | undefined) =>
    value === null || value === undefined ? "-" : formatKg(Number(value));
  const optionalUnits = (value: string | number | null | undefined) =>
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description="Registre perdas e calcule custo, filme aproveitado e resultado financeiro da embalagem."
      />
      <Card>
        <div className="grid gap-4 md:grid-cols-5">
          ...
          />
          <Select
            label="Produto (preco aprovado)"
            value={productId}
            onChange={setProductId}
            options={products
              .filter(
                (product) =>
                  !product.defaultSector ||
                  product.defaultSector.code === sector,
              )
              .map((product) => ({
                value: product.id,
          ...
          <NumberInput
            label="Filme T1 (kg)"
            value={filmShift1Kg}
            onChange={setFilmShift1Kg}
          />
          <NumberInput
            label="Filme T2 (kg)"
            value={filmShift2Kg}
            onChange={setFilmShift2Kg}
          />
          <NumberInput
            label="Caixas perdidas (un)"
            value={boxLossUnits}
            onChange={() => undefined}
            disabled
          />
          <NumberInput
            label="Caixas T1 (un)"
            value={boxLossShift1Units}
            onChange={setBoxLossShift1Units}
          />
          <NumberInput
            label="Caixas T2 (un)"
            value={boxLossShift2Units}
            onChange={setBoxLossShift2Units}
          />
        </div>
        <div className="mt-4 flex flex-wrap gap-3">
          <Input label="Motivo" value={reason} onChange={setReason} />
          <NumberInput
            label="Caixas produzidas (filme)"
            value={packedBoxes}
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <StatCard
          label="Perdas da semana"
          value={formatKg(total)}
          hint={lossGoal.hint}
          status={lossGoal.status}
        />
        <StatCard
          label="Registros aprovados"
          value={`${approvedEntries.length} de ${entries.length}`}
        />
        <StatCard
          label="Meta semanal"
          value={
            uniqueLossGoal ? formatKg(uniqueLossGoal.target) : "Não definida"
          }
          hint={lossGoal.hint}
          status={lossGoal.status}
        />
  ...
  return (
    <label className="space-y-2">
      ...
      <input
        ...
        min="0"
        disabled={disabled}
        className="w-full rounded-md border border-[var(--line)] bg-white/5 px-3 py-2 text-sm outline-none focus:border-cyan-300/60 disabled:opacity-60"
        value={value}
